...
/**
 * Connector class for interfacing with a kdb+ process. This class is essentially a
 * serializer/deserializer of java types to/from the kdb+ ipc wire format, enabling remote method
 * invocation in kdb+ via tcp/ip.
 *
 * <p>Further information can be found at <a
 * href="https://code.kx.com/q/interfaces/java-client-for-q/">https://code.kx.com/q/interfaces/java-client-for-q/</a>
 *
 * <p>To begin with, a connection may be established to a listening kdb+ process via the constructor
 * <code>c connection=new c("localhost",5000);</code>
 *
 * <p>There are then 3 methods available for interacting with the connection:
 *
 * <ol>
...
public class c {
  ...
  public Socket s;
  /**
   * {@code i} is the {@code DataInputStream} of the socket used to read data from the remote kdb+
   * process.
   */
  ...
  /**
   * {@code b} is the buffer used to store the incoming message bytes from the remote prior to
   * de-serialization
   */
  byte[] rBuff;
  ...
  /**
   * Indicates whether the current connection is to a local interface. Tested when considering
   * whether to compress an outgoing message.
   */
  boolean isLoopback;
  /**
   * Indicates whether messages should be candidates for compressing before sending (given
   * uncompressed serialized data also has a length greater than 2000 bytes and connection is not
   * localhost)
   */
  boolean zip;
  /**
   * Sets whether or not to consider compression on outgoing messages (given uncompressed serialized
   * data also has a length greater than 2000 bytes and connection is not localhost)
   *
  ...
  /**
   * Initializes a new {@link c} instance by acting as a server, blocking till a client connects and
   * authenticates using the KDB+ protocol. This object should be used for a single client
   * connection. A new instance should be created for each new client connection.
   *
   * @param s {@link ServerSocket} to accept connections on using kdb+ IPC protocol.
   * @param a {@link IAuthenticate} instance to authenticate incoming connections. Accepts all
   *     incoming connections if {@code null}.
  ...
  /**
   * Initializes a new {@link c} instance by acting as a server, and blocks while waiting for a new
   * client connection. A new instance should be created for each new client connection.
   *
   * @param s {@link ServerSocket} to accept connections on using kdb+ IPC protocol.
   * @throws IOException an I/O error occurs.
   */
  ...
  /**
   * Initializes a new {@link c} instance and connects to KDB+ over TCP with optional TLS support
   * for encryption.
   *
   * @param host Host of remote q process
   * @param port Port of remote q process
  ...
  /**
   * Initializes a new {@link c} instance and connects to KDB+ over TCP, using {@code user.name}
   * system property for username and password criteria. The {@code user.name} system property
   * should be set to a value in the "username:password" format for remote authorization
   *
   * @param host Host of remote q process
   * @param port Port of remote q process
   * @throws KException if access denied
  ...
  /**
   * Initializes a new {@link c} instance for the purposes of serialization only, no connection is
   * instantiated to/from a KDB+ process
   */
  public c() {
    ipcVersion = '\3';
    isLoopback = false;
  ...
  public static class Month implements Comparable<Month> {
    ...
    public int i;
    /**
     * Create a KDB+ representation of 'month' type from the q language (a month value is the count
     * of months since the beginning of the millennium. Post-milieu is positive and pre is negative)
     *
  ...
  /**
   * {@code Minute} represents kdb+ minute type, which is a time represented as the number of
  ...
  public static class Minute implements Comparable<Minute> {
    ...
    public int i;
    /**
     * Create a KDB+ representation of 'minute' type from the q language (point in time represented
  ...
  /**
   * {@code Second} represents kdb+ second type, which is a point in time represented in seconds
   * since midnight.
   */
  public static class Second implements Comparable<Second> {
    ...
    public int i;
    /**
     * Create a KDB+ representation of 'second' type from the q language (point in time represented
  ...
  public static class Timespan implements Comparable<Timespan> {
    ...
    public long j;
    /**
     * Create a KDB+ representation of 'timespan' type from the q language (point in time
     * represented in nanoseconds since midnight)
     *
  ...
  public static class Dict {
    ...
    public Object y;
    /**
     * Create a representation of the KDB+ dictionary type, which is a mapping between keys and
     * values
     *
     * @param keys Keys to store. Should be an array type when using multiple values.
     * @param vals Values to store. Index of each value should match the corresponding associated
     *     key. Should be an array type when using multiple values.
     */
  ...
  /**
   * {@code Flip} represents a kdb+ table (an array of column names, and an array of arrays
   * containing the column data). q tables are column-oriented, in contrast to the row-oriented
   * tables in relational databases. An introduction can be found at <a
  ...
  /**
   * {@code KException} is used to indicate there was an error generated by the remote process
   * during the processing of a sync message or if the connection failed due to access credentials.
   * Network errors are reported as IOException.
   */
  public static class KException extends Exception {
    private static final long serialVersionUID =
        1554689726734674152L; // autogenerated by serialver kx.c.KException
   ...
   */
  void w(UUID uuid) {
    if (ipcVersion < 3) throw new RuntimeException("Guid not valid pre kdb+3.0");
   ...
   */
  void w(Timespan n) {
    if (ipcVersion < 1) throw new RuntimeException("Timespan not valid pre kdb+2.6");
    w(n.j);
  }
  ...
  java.util.Date rz() {
    ...
    return new java.util.Date(
        Double.isNaN(f) ? nj : gl(MILLS_BETWEEN_1970_2000 + Math.round(8.64e7 * f)));
  }
  /**
   * Write Date to serialization buffer in big endian format
   *
  ...
  Timestamp rp() {
    ...
    Timestamp p =
        new Timestamp(timeAsLong == nj ? timeAsLong : gl(MILLS_BETWEEN_1970_2000 + 1000 * d));
    if (timeAsLong != nj) p.setNanos((int) (timeAsLong - NANOS_IN_SEC * d));
    return p;
  }
  /**
  ...
  void w(Timestamp p) {
    ...
    if (ipcVersion < 1) throw new RuntimeException("Timestamp not valid pre kdb+2.6");
    w(
        millsSince1970 == nj
            ? millsSince1970
            : 1000000 * (lg(millsSince1970) - MILLS_BETWEEN_1970_2000) + p.getNanos() % 1000000);
  }
  /**
  ...
  String rs() throws UnsupportedEncodingException {
    ...
    while (rBuff[rBuffPos++] != 0)
      ;
    return (startPos == rBuffPos - 1)
        ? ""
        : new String(rBuff, startPos, rBuffPos - 1 - startPos, encoding);
  }
  /**
  ...
  void w(String s) throws UnsupportedEncodingException {
    if (s != null) {
      ...
      for (int idx = 0; idx < byteLen; idx++) w(bytes[idx]);
    }
    wBuff[wBuffPos++] = 0;
  }
  /**
  ...
  Object r() throws UnsupportedEncodingException {
    ...
    if (t > 99) {
      ...
      if (t < 104) return rBuff[rBuffPos++] == 0 && t == 101 ? null : "func";
      if (t > 105) r();
    ...
    switch (t) {
      case 0:
        ...
        for (; i < n; i++) objArr[i] = r();
        return objArr;
      case 1:
        boolean[] boolArr = new boolean[n];
        for (; i < n; i++) boolArr[i] = rb();
        return boolArr;
      case 2:
        UUID[] uuidArr = new UUID[n];
        for (; i < n; i++) uuidArr[i] = rg();
        return uuidArr;
      case 4:
        byte[] byteArr = new byte[n];
        for (; i < n; i++) byteArr[i] = rBuff[rBuffPos++];
        return byteArr;
      case 5:
        short[] shortArr = new short[n];
        for (; i < n; i++) shortArr[i] = rh();
        return shortArr;
      case 6:
        int[] intArr = new int[n];
        for (; i < n; i++) intArr[i] = ri();
        return intArr;
      case 7:
        long[] longArr = new long[n];
        for (; i < n; i++) longArr[i] = rj();
        return longArr;
      case 8:
        float[] floatArr = new float[n];
        for (; i < n; i++) floatArr[i] = re();
        return floatArr;
      case 9:
        double[] doubleArr = new double[n];
        for (; i < n; i++) doubleArr[i] = rf();
        return doubleArr;
      case 10:
  ...
  /**
   * Gets the numeric type of the supplied object used in kdb+ (distict supported data types in KDB+
   * can be identified by a numeric).&nbsp; See data type reference <a
   * href="https://code.kx.com/q/basics/datatypes/">https://code.kx.com/q/basics/datatypes/</a>. For
   * example, an object of type java.lang.Integer provides a numeric type of -6.
   *
   * @param x Object to get the numeric type of
   * @return kdb+ type number for an object
   */
  public static int t(final Object x) {
    if (x instanceof Boolean) return -1;
    if (x instanceof UUID) return -2;
    if (x instanceof Byte) return -4;
    if (x instanceof Short) return -5;
    if (x instanceof Integer) return -6;
    if (x instanceof Long) return -7;
    if (x instanceof Float) return -8;
    if (x instanceof Double) return -9;
    if (x instanceof Character) return -10;
    if (x instanceof String) return -11;
    if (x instanceof Date) return -14;
    if (x instanceof Time) return -19;
    if (x instanceof Timestamp) return -12;
    if (x instanceof java.util.Date) return -15;
    if (x instanceof Timespan) return -16;
    if (x instanceof Month) return -13;
    if (x instanceof Minute) return -17;
    if (x instanceof Second) return -18;
    if (x instanceof boolean[]) return 1;
    if (x instanceof UUID[]) return 2;
    if (x instanceof byte[]) return 4;
    if (x instanceof short[]) return 5;
    if (x instanceof int[]) return 6;
    if (x instanceof long[]) return 7;
    if (x instanceof float[]) return 8;
    if (x instanceof double[]) return 9;
    if (x instanceof char[]) return 10;
    if (x instanceof String[]) return 11;
    if (x instanceof Date[]) return 14;
    if (x instanceof Time[]) return 19;
    if (x instanceof Timestamp[]) return 12;
    ...
    if (x instanceof Minute[]) return 17;
    if (x instanceof Second[]) return 18;
    if (x instanceof Flip) return 98;
    if (x instanceof Dict) return 99;
    return 0;
  }
  /**
   * "number of bytes from type." A helper for nx, to assist in calculating the number of bytes
   * required to serialize a particular type.
   */
  ...
  public static int n(final Object x) throws UnsupportedEncodingException {
    if (x instanceof Dict) return n(((Dict) x).x);
    if (x instanceof Flip) return n(((Flip) x).y[0]);
    return x instanceof char[]
        ? new String((char[]) x).getBytes(encoding).length
        : Array.getLength(x);
  }
  /**
  ...
  public int nx(Object x) throws UnsupportedEncodingException {
    int type = t(x);
    if (type == 99) return 1 + nx(((Dict) x).x) + nx(((Dict) x).y);
    if (type == 98) return 3 + nx(((Flip) x).x) + nx(((Flip) x).y);
    if (type < 0) return type == -11 ? 2 + ns((String) x) : 1 + nt[-type];
    int numBytes = 6;
    int numElements = n(x);
    if (type == 0 || type == 11)
      for (int idx = 0; idx < numElements; ++idx)
        numBytes += type == 0 ? nx(((Object[]) x)[idx]) : 1 + ns(((String[]) x)[idx]);
    else numBytes += numElements * nt[type];
    return numBytes;
  }
  /**
  ...
  void w(Object x) throws UnsupportedEncodingException {
    ...
    if (type == 10) {
      ...
      for (; i < n; ++i)
        if (type == 0) w(((Object[]) x)[i]);
        else if (type == 1) w(((boolean[]) x)[i]);
        else if (type == 2) w(((UUID[]) x)[i]);
        else if (type == 4) w(((byte[]) x)[i]);
        else if (type == 5) w(((short[]) x)[i]);
        else if (type == 6) w(((int[]) x)[i]);
        else if (type == 7) w(((long[]) x)[i]);
        else if (type == 8) w(((float[]) x)[i]);
        else if (type == 9) w(((double[]) x)[i]);
        else if (type == 11) w(((String[]) x)[i]);
        else if (type == 12) w(((Timestamp[]) x)[i]);
        else if (type == 13) w(((Month[]) x)[i]);
        else if (type == 14) w(((Date[]) x)[i]);
        else if (type == 15) w(((java.util.Date[]) x)[i]);
        else if (type == 16) w(((Timespan[]) x)[i]);
        else if (type == 17) w(((Minute[]) x)[i]);
        else if (type == 18) w(((Second[]) x)[i]);
        else w(((Time[]) x)[i]);
  }
   ...
   * @param zip true if to attempt compress serialised output (given uncompressed serialized data
   *     also has a length greater than 2000 bytes and connection is not localhost)
  ...
  public byte[] serialize(int msgType, Object x, boolean zip) throws IOException {
    ...
    synchronized (outStream) {
      ...
      w(length);
      w(x);
      if (zip && wBuffPos > 2000 && !isLoopback) compress();
      return wBuff;
    }
  }
  ...
  /**
   * Sends a response message to the remote kdb+ process. This should be called only during
   * processing of an incoming sync message.
   *
  ...
  /**
   * Sends an error as a response message to the remote kdb+ process. This should be called only
   * during processing of an incoming sync message.
   *
  ...
  /**
   * Sends an async message to the remote kdb+ process. This blocks until the serialized data has
   * been written to the socket. On return, there is no guarantee that this msg has already been
   * processed by the remote process.
   *
   ...
   * Sends an async message to the remote kdb+ process. This blocks until the serialized data has
   * been written to the socket. On return, there is no guarantee that this msg has already been
   * processed by the remote process. Use this to invoke a function in kdb+ which takes a single
   * argument and does not return a value. e.g. to invoke f[x] use ks("f",x); to invoke a lambda,
   * use ks("{x}",x);
   *
   ...
   * Sends an async message to the remote kdb+ process. This blocks until the serialized data has
   * been written to the socket. On return, there is no guarantee that this msg has already been
   * processed by the remote process. Use this to invoke a function in kdb+ which takes 2 arguments
   * and does not return a value. e.g. to invoke f[x;y] use ks("f",x,y); to invoke a lambda, use
   * ks("{x+y}",x,y);
   *
   * @param s The name of the function, or a lambda itself
   ...
   * Sends an async message to the remote kdb+ process. This blocks until the serialized data has
   * been written to the socket. On return, there is no guarantee that this msg has already been
   * processed by the remote process. Use this to invoke a function in kdb+ which takes 3 arguments
   * and does not return a value. e.g. to invoke f[x;y;z] use ks("f",x,y,z); to invoke a lambda, use
   * ks("{x+y+z}",x,y,z);
   *
   * @param s The name of the function, or a lambda itself
   * @param x The first argument to the function named in s
  ...
  /**
   * Reads an incoming message from the remote kdb+ process. This blocks until a single message has
   * been received and deserialized. This is called automatically during a sync request via k(String
   * s,..). It can be called explicitly when subscribing to a publisher.
   *
  ...
  /**
   * MsgHandler interface for processing async or sync messages during a sync request whilst
   * awaiting a response message which contains a default implementation
   */
  ...
  /**
   * {@code msgHandler} is used for handling incoming async and sync messages whilst awaiting a
   * response message to a sync request
   */
  ...
  /**
   * {@code collectResponseAsync} is used to indicate whether k() should leave the reading of the
   * associated response message to the caller via readMsg();
   */
  ...
  /**
   * Sends a sync message to the remote kdb+ process. This blocks until the message has been sent in
   * full, and, if a MsgHandler is set, will process any queued, incoming async or sync message in
   * order to reach the response message. If the caller has already indicated via {@code
   * setCollectResponseAsync} that the response message will be read async, later, then return
   * without trying to read any messages at this point; the caller can collect(read) the response
  ...
  public synchronized Object k(Object x) throws KException, IOException {
    w(1, x);
    if (collectResponseAsync) return null;
    while (true) {
      Object[] msg = readMsg();
      if (msgHandler == null
          || (byte) msg[0]
              == (byte) 2) // if there's no handler or the msg is a response msg, return it
      return msg[1];
  ...
  /**
   * Sends a sync message to the remote kdb+ process. This blocks until the message has been sent in
   * full, and a message is received from the remote; typically the received message would be the
   ...
   * Sends a sync message to the remote kdb+ process. This blocks until the message has been sent in
   * full, and a message is received from the remote; typically the received message would be the
   * corresponding response message. Use this to invoke a function in kdb+ which takes a single
   * argument and returns a value. e.g. to invoke f[x] use k("f",x); to invoke a lambda, use
   * k("{x}",x);
   *
   * @param s The name of the function, or a lambda itself
   * @param x The argument to the function named in s
   ...
   * Sends a sync message to the remote kdb+ process. This blocks until the message has been sent in
   * full, and a message is received from the remote; typically the received message would be the
   * corresponding response message. Use this to invoke a function in kdb+ which takes arguments and
   * returns a value. e.g. to invoke f[x;y] use k("f",x,y); to invoke a lambda, use k("{x+y}",x,y);
   *
   * @param s The name of the function, or a lambda itself
   * @param x The first argument to the function named in s
   * @param y The second argument to the function named in s
   ...
   * Sends a sync message to the remote kdb+ process. This blocks until the message has been sent in
   * full, and a message is received from the remote; typically the received message would be the
   * corresponding response message. Use this to invoke a function in kdb+ which takes 3 arguments
   * and returns a value. e.g. to invoke f[x;y;z] use k("f",x,y,z); to invoke a lambda, use
   * k("{x+y+z}",x,y,z);
   *
   * @param s The name of the function, or a lambda itself
   * @param x The first argument to the function named in s
   ...
   * See data type reference <a
   * href="https://code.kx.com/q/basics/datatypes/">https://code.kx.com/q/basics/datatypes/</a> For
   * example {@code "".equals(NULL[11])}
   */
  ...
  /**
   * Gets a null object for the type indicated by the character.&nbsp; See data type reference <a
   * href="https://code.kx.com/q/basics/datatypes/">https://code.kx.com/q/basics/datatypes/</a>
   *
  ...
  /**
   * Tests whether an object represents a KDB+ null for its type, for example qn(NULL('j')) should
   * return true
   *
   ...
   * @return object at index, or null if the object value represents a KDB+ null value for its type
   */
   ...
   * @param x The array to index
   * @param i The offset to index at
   * @param y The object to set at index i. null can be used if you wish to set the KDB+ null
   *     representation of the type (e.g. null would populate an empty string if x was an array of
   *     Strings)
   */
   ...
   *
   * <p>A keyed table(a.k.a. Flip) is a dictionary where both key and value are tables themselves.
   * For ease of processing, this method, td, table from dictionary, can be used to remove the key.
   *
  ...
  public static Flip td(Object tbl) throws UnsupportedEncodingException {
    if (tbl instanceof Flip) return (Flip) tbl;
    Dict d = (Dict) tbl;
    Flip a = (Flip) d.x;
    Flip b = (Flip) d.y;
    int m = n(a.x);
  ...
  /**
   * @deprecated Prints time in milliseconds between invocations on the function. Will be removed in
   *     future releases.
   */
  @Deprecated
  public static void tm() {
    ...
    tmCallTime = t();
    if (prevCallTime > 0) O(tmCallTime - prevCallTime);
  }
  /**
