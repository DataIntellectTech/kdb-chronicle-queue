...
public class ChronicleKdbAdapter {
    ...
    public int processMessages(AdapterProperties adapterProperties) {
        ...
        try {
            ...
            tooLongSinceLastMsg = adapterTimer.tooLongSinceLastMsg(adapterProperties.getKdbEnvelopeWaitTime());
            ...
            while (keepRunning) {
                ...
                tailer.readDocument(q -> q.read(adapterProperties.getAdapterMessageType())
                        .marshallable(
                                m -> {
                                    ...
                                    finish = System.nanoTime() - start;
                                    LOG.trace("TIMING: Added msg -> envelope obj in {} seconds", finish / 1e9);
            ...
            LOG.info("Stopping Chronicle kdb Adapter. {} msgs stored in this cycle ({} seconds)", howManyStored, processFinish / 1e9);
            return ret;
    ...
    private boolean saveCurrentEnvelope(AdapterProperties adapterProperties, KdbEnvelope envelope){
        ...
        if (kdbConnector.saveMessage(adapterProperties, envelope)) {
            ...
            finish = System.nanoTime() - start;
            LOG.info("TIMING: Stored {} messages (up to index: {}) in {} seconds", howManyRead, tailerIndex, finish / 1e9);
